...
def collect_all_metric_columns(files: List[Path]) -> List[str]:
    ...
    value_fields = ["unit", "Pred", "Best", "Best_P", "Act1", "Act2", "Act3", "Act4", "Act5", "Act6"]
...
def main():
    IN_PARSED_DIR = Path("./out_stage02_parsed/parsed_json")  # 如果你 step02 没改目录，就改回 out_stage02_parsed/parsed_json
    OUT_DIR = Path("./out_stage03_csv")
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    out_csv = OUT_DIR / "ventilation_reports_wide.csv"

    files = sorted(IN_PARSED_DIR.glob("*.parsed.json"))
    if not files:
        raise FileNotFoundError(f"No parsed json found in {IN_PARSED_DIR}")

    base_cols = ["file_stem", "name", "sex", "age", "height_cm", "weight_kg", "id", "test_no", "impression"]
