    ...
    for line in lines[:30]:
        ...
        for t in line:
            ...
            if re.fullmatch(r"\d{10,}", s):
                patient["test_no"] = s
    # ===== footer: Date/Time, operator/reviewer =====
    footer = parse_footer(lines)
    patient.update({k: v for k, v in footer.items() if v})

    return patient


def parse_footer(lines: List[List[Dict[str, Any]]]) -> Dict[str, Any]:
    """
    解析页脚信息：
    - Date / Time（可能出现 1 组或 2 组，这里优先取“最靠下的第一组”作为 report_date/report_time）
    - 操作者/审核者（如果 OCR 能给出名字就取）
    """
    out: Dict[str, Any] = {}

    # 把每一行拼起来方便扫
    joined_lines = [" ".join(t["text"].strip() for t in line if t["text"].strip()) for line in lines]

    # -------- 1) Date/Time：取页面底部最先遇到的一组 --------
    # 从下往上找，保证拿到红框那组（更靠下）
    date_pat = re.compile(r"\b(\d{2}/\d{2}/\d{2})\b")
    time_pat = re.compile(r"\b(\d{1,2}:\d{2}:\d{2}\s*[AP]M)\b", re.I)

    found_date = None
    found_time = None
    for s in reversed(joined_lines):
        ...
        if found_time is None:
            ...
            if m:
                # 统一一下 AM/PM 格式
                found_time = m.group(1).replace(" ", "").upper()
        if found_date and found_time:
            break

    if found_date:
        out["report_date"] = found_date
    if found_time:
        out["report_time"] = found_time

    # -------- 2) 操作者/审核者（能抓到就抓）--------
    # 你 raw 里出现过 “操作者：方教敏”“审核者：方教敏”
    # 这里用“含关键字的行 + 冒号后面的内容”抓
    def grab_after_keyword(keyword: str) -> Optional[str]:
    ...
    op = grab_after_keyword("操作者")
    rv = grab_after_keyword("审核者")
    if op:
        out["operator"] = op
    if rv:
        out["reviewer"] = rv
    ...
    for idx, line in enumerate(lines):
        ...
        for t in line:
            ...
            if s == "预计值":
                ...
                colx["Act1"] = t["cx"]
            elif s in ("% (前/预)", "%(前/预)", "％(前/预)"):
                colx["Act1_P"] = t["cx"]
            elif s == "药后值":
                colx["Act2"] = t["cx"]
            elif s in ("% (后/预)", "%(后/预)", "％(后/预)"):
                colx["Act2_P"] = t["cx"]
            elif s in ("改善率", "改善"):
                colx["Delta_P"] = t["cx"]

        # 可选：如果 %(前/预) 或 %(后/预) 被 OCR 写法搞变形，允许缺失
        return idx, colx
